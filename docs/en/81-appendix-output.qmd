...
Only `$OutPort` in `scripts\windows\lib\common.ps1` (default `2101`) is published. If you want to change it, see [Change the port in use](90-troubleshooting.qmd#sec-troubleshooting-port-change).
:::
...
Connect the output device via USB, then, in an administrator PowerShell ([Run PowerShell as administrator](20-install-windows.qmd#sec-win-wsl2-powershell)), list the connected USB devices.
...
## If it doesn't work {.unnumbered}

- [Troubleshooting](90-troubleshooting.qmd)
