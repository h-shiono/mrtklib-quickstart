...
次に、Windows Subsystem for Linux 2 (WSL2) を有効化し、Ubuntu ディストリビューションをインストールします。
...
::: {.callout-note}
本ガイドでは、受信機の USB を WSL へアタッチするために usbipd-win を使用します。`usbipd attach --wsl` は実体のある WSL2 ディストリビューションを必要とするため、Docker Desktop のバックエンド（`docker-desktop`）とは別に、Ubuntu のインストールが必要です。
:::
