...
## なぜこの設定が必要か {#sec-appendix-why}

MADOCA-PPP による測位は、受信機自身が測った**観測量**（擬似距離・搬送波位相）に対して QZSS L6 から届く**補正情報**を適用し、計算を行います。
測位演算を行うのはコンテナ内の MRTKLIB エンジンなので、受信機には**両方を SBF 形式で出力させる**必要があります。
以下の設定は、そのための最小構成です。

| 設定 | なぜ必要か |
| --- | --- |
| **QZSL6 信号を Tracking** | L6 信号を追尾・受信するために必要です。`QZSRawL6E`（MADOCA-PPP：軌道・クロック・バイアス補正）と `QZSRawL6D`（広域電離圏補正）が得られます。 |
| **全コンステレーションを Tracking (All)** | 補強対象の GPS / GLONASS / Galileo / BeiDou / QZSS の観測量を得るために必要です。 |
| **SBF を `USB1` に出力** | コンテナは USB シリアル経由で SBF を読みます。`COM1` は物理シリアルで USB には出ないため `USB1` を選びます。 |
| **`Support` メッセージ群** | 生観測量（`MeasEpoch`）・航法データ・`QZSRawL6D/E` をまとめて含み、PPP に必要な入力が揃います。 |
| **`Boot` に保存** | 電源を入れ直しても設定が残るようにするために必要です。 |

以下は、上記を RxControl の GUI で手動設定する手順です（Windows / Linux 共通、画面は Windows）。

## 受信機の接続（Windows, Linux） {#sec-appendix-connect}

!!! warning "注意"

    先に「RxTools」のインストール（[Windows](20-install-windows.md#sec-win-rxtools)）を完了させてから本節をお読みください。

USB ケーブルを使用して、受信機を Windows または Linux の PC に接続します。
正常に接続されると、受信機の PWR LED が赤色に点灯します。

## 受信機の設定（Windows, Linux） {#sec-appendix-config}
...
### RxControl の起動 {#sec-appendix-config-rxcontrol}
...
### 受信衛星・信号の設定 {#sec-appendix-config-tracking}
...
### SBF 出力の設定 {#sec-appendix-config-sbf-output}
...
### 受信機設定の保存 {#sec-appendix-config-save}
...
## 受信機の設定（macOS） {#sec-appendix-config-macos}
