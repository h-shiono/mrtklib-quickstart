...
公開されているのは `scripts\windows\lib\common.ps1` の `$OutPort`（既定では `2101`）だけです。変更したい場合は [使用するポートを変更する](90-troubleshooting.qmd#sec-troubleshooting-port-change) を参照してください。
:::
